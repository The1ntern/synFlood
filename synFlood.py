...
def main():
	...
	if not destinationIp or not interface:
		...
		for cpus in range(multiprocessing.cpu_count()):
			synSender = Process(name="synFlood%i" % (cpus), target=sendSYNs, args=(destinationIp, targetPort, interface))
